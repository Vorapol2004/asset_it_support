...
public interface EquipmentRepository extends JpaRepository<Equipment, Integer> {
    ...
    @Query(value = """
            ...
            LEFT JOIN equipment_status es ON es.id = e.equipment_status_id
            LEFT JOIN lot l ON l.id = e.lot_id
            LEFT JOIN lot_type lt ON lt.id = l.lot_type_id
            
            WHERE 1 = 1
                AND (:equipmentStatusId IS NULL OR e.equipment_status_id = :equipmentStatusId)
            ...
            ORDER BY
                l.purchase_date DESC, l.id;
            """, nativeQuery = true)
    List<EquipmentView> search(
            @Param("equipmentStatusId") Integer equipmentStatusId,
